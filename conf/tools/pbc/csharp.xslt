...
>
  ...
  <xsl:variable name="optionIgnoredefault" select="$ignoredefault='true'"/>
...
// Note: requires additional types generated from: <xsl:value-of select="."/></xsl:template>
  ...
  <xsl:template match="FieldDescriptorProto[label='LABEL_OPTIONAL' or not(label)]">
	...
	<xsl:if test="not($optionIgnoredefault) or not(default_value)">
    [global::System.ComponentModel.DefaultValue(<xsl:value-of select="$defaultValue"/>)]</xsl:if></xsl:if><!--
    --><xsl:if test="$optionXml">
    <xsl:choose>
        <xsl:when test="type='TYPE_MESSAGE'">
    [global::System.Xml.Serialization.XmlElement(@"<xsl:value-of select="name"/>")]
        </xsl:when>
        <xsl:otherwise>
    ...
    [global::System.Runtime.Serialization.DataMember(Name=@"<xsl:value-of select="name"/>", IsRequired = false)]
  ...
  <xsl:template match="FieldDescriptorProto[label='LABEL_REQUIRED']">
    ...
    [global::System.Runtime.Serialization.DataMember(Name=@"<xsl:value-of select="name"/>", IsRequired = true)]
    </xsl:if><xsl:call-template name="WriteGetSet">
